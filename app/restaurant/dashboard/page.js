...
import { Menu, X, Plus, Edit, Trash2, Clock, User, Phone, Mail, DollarSign, Package, ChefHat, Bell, CheckCircle, XCircle, Pause } from "lucide-react";
...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-600 text-sm">Manage your orders and menu</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2 bg-orange-100 px-3 py-2 rounded-lg">
                ...
                <span className="text-orange-800 font-semibold">{pendingOrders.length} Active Orders</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex">
        {/* Sidebar */}
        <div className={`${isSidebarOpen ? 'w-80' : 'w-0'} overflow-hidden transition-all duration-300 bg-white shadow-xl border-r border-gray-200`}>
          <div className="p-6">
            ...
            {activeTab === 'menu' && (
              <div className="mt-6">
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {menu.map((item) => (
                    <div key={item._id} className="p-3 bg-gray-50 rounded-lg border">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6 border border-blue-100">
              <div className="flex items-center mb-6">
                ...
                <div>
                  ...
                  <p className="text-gray-600">Manage incoming and ongoing orders</p>
                </div>
              </div>
              ...
              {orders.length === 0 ? (
                ...
                <div className="grid gap-6">
                  {orders.map((order) => (
                    <div key={order._id} className="border border-gray-200 rounded-xl p-6 bg-gradient-to-r from-white to-gray-50 hover:shadow-lg transition-shadow">
                      {/* Order Header */}
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <div className="text-right">
                          <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium border ${getOrderStatusColor(order.status)}`}>
                            {order.status.charAt(0).toUpperCase() + order.status.slice(1)}
                          </div>
                          <p className="text-2xl font-bold text-green-600 mt-2">₹{order.totalAmount}</p>
                        </div>
                      </div>
                        ...
                        <div className="space-y-2">
                          {order.items && order.items.map((i, index) => (
                            <div key={index} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                              <div>
                                <span className="font-medium text-gray-800">{i.item?.name || 'Unknown Item'}</span>
                                <span className="text-sm text-gray-500 ml-2">x{i.quantity}</span>
                              </div>
                              <span className="font-semibold text-green-600">₹{(i.item?.price || 0) * i.quantity}</span>
                            </div>
                          ))}
                        </div>
                      </div>
              ...
              <div className="bg-white rounded-2xl shadow-xl p-6 border border-purple-100">
                ...
                {menu.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {menu.map((item) => (
                      <div key={item._id} className="bg-gradient-to-br from-white to-purple-50 border border-purple-100 rounded-xl p-6 hover:shadow-lg transition-shadow">
